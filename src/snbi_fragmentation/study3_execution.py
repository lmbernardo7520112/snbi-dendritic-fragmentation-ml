...
def read_text(root, relative):
    require(relative in FREEZE_FILES + INHERITED_FILES or relative in {
...
def emit(root, name, value):
    ...
    directory = safe_path(root, EVIDENCE)
    directory.mkdir(parents=True, exist_ok=True)
    path = safe_path(root, EVIDENCE + "/" + name)
...
def _execute(root):
    ...
    proof, manifest, design = preflight(root)
    ...
    receipt_sha = emit(root, RECEIPT, {**proof, "scientific_receipt_created": True, "attempt": 1,
    ...
    try:
        ...
        selection_digest = emit(root, "TEMPORAL_SELECTION_LEDGER.json", {"selections": selections,
    ...
    terminal = {"STUDY3": "PASS" if failure is None else "BLOCKED_PARTIAL_EXECUTION",
        ...
        "DISTINCT_FITS": budget.snapshot()["distinct_fits_completed"], **counts}
    ...
    outputs = {
        ...
        "commands.json": {"entrypoint": "scripts/run_study3.py", "invocations": 1,
    ...
    hashes = {name: emit(root, name, value) for name, value in outputs.items()}
    ...
    emit(root, "post-run-hashes.sha256", "".join(f"{sha}  {name}\n" for name, sha in sorted(hashes.items())))
