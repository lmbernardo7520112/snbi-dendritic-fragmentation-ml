"""TI3-D one-use receipt, phase-gated FINAL and accounted native I/O.

Previous authorities remain consumed. FINAL admission requires all ten training
reads and a durable logical-fit record before any FINAL path is examined.
"""
...
BASE = "artifacts/evidence/TI3_D_FINAL"
...
FINAL_FRAMES = frozenset({("ESM1", 293), ("ESM2", 293), ("ESM4", 295), ("ESM5", 295)})
ALL_FRAMES = dict(ALLOWED_FRAMES)
ALL_FRAMES.update({key: "FINAL_TEST" for key in FINAL_FRAMES})
FIT_PATH = BASE + "/fit-freeze.json"
FINAL_ACCESS_PATH = BASE + "/final-access-receipt.json"
...
def validate_admission(record):
    """Check the explicit fourteen records; the reader separately gates FINAL."""
    require(isinstance(record, dict), "SOURCE_METADATA_REQUIRED")
    require(record.get("split") in ("TRAIN", "DEVELOPMENT", "FINAL_TEST"), "UNKNOWN_SPLIT_DENIED")
    ...
    require(type(frame) is int and (source, frame) in ALL_FRAMES, "FRAME_NOT_AUTHORIZED")
    require(ALL_FRAMES[(source, frame)] == record["split"], "FRAME_SPLIT_MISMATCH")
...
def build_inventory(manifest, pilot):
    ...
    records remain phase-gated; inventory membership alone grants no access.
    """
...
def validate_inventory(inventory):
    ...
    require(seen == set(ALLOWED_FRAMES) | set(FINAL_FRAMES), "INVENTORY_ALLOWLIST_MISMATCH")
...
def arm_execution(root, manifest_bytes, inventory, authority, head):
    """Consume a new exclusive receipt after the caller's D1/CI preflight."""
    ...
    require(digest_string(head, 40) and authority.get("head_sha") == head, "D1_HEAD_MISMATCH")
    ...
    allowed = {(r["source_id"], r["frame_index"]): r for r in inventory }
    ...
    try:
        ...
        receipt = {"schema": "TI3D-EXECUTION-RECEIPT-1", "state": "CONSUMED_BEFORE_NATIVE_BYTES",
        ...
        for split in ("TRAIN", "DEVELOPMENT"):
            for source in manifest["source_inventory"]:
                if source["split"] == split:
                    training_ids.extend(sorted(s["sample_id"] for s in manifest["samples"]
                        if (s["source_id"], s["frame_index"]) == (source["source_id"], source["frame_index"])))
        reader = NativeReader(_ARM_TOKEN, root_fd, inventory, allowed, training_ids, head)
        root_fd = None
        return reader
    except OSError:
...
class NativeReader:
    ...
    def __init__(self, token, root_fd, inventory, allowed, training_ids, head):
        require(token is _ARM_TOKEN, "EXCLUSIVE_RECEIPT_REQUIRED")
        self._root_fd, self._allowed = root_fd, copy.deepcopy(allowed)
        self._training_ids, self._head = copy.deepcopy(training_ids), head
        self._failed = self._closed = False
        self._final_authorized = self._final_started = self._final_consumed = False
        self._fit_sha256 = None
        self._attempted = set()
    ...
    def authorize_final(self, fit_sha256):
        """Open only the new text fit record; require all ten training hashes."""
        require(not self._closed and not self._failed and not self._final_authorized,
                "FINAL_AUTHORIZATION_STATE_INVALID")
        require(all(r["verified"] for r in self._entries.values() if r["split"] != "FINAL_TEST"),
                "FINAL_BEFORE_TRAINING_READS_DENIED")
        require(digest_string(fit_sha256), "FIT_FREEZE_HASH_REQUIRED")
        parent, fd = None, None
        try:
            parent, name = _parent_fd(self._root_fd, FIT_PATH)
            fd = os.open(name, os.O_RDONLY | os.O_NOFOLLOW | os.O_CLOEXEC, dir_fd=parent)
            require(stat.S_ISREG(os.fstat(fd).st_mode), "FIT_FREEZE_NOT_REGULAR")
            raw = b""
            while True:
                ...
                if not chunk:
                    break
                raw += chunk
                require(len(raw) <= 1048576, "FIT_FREEZE_TOO_LARGE")
            require(hashlib.sha256(raw).hexdigest() == fit_sha256, "FIT_FREEZE_HASH_MISMATCH")
            record = json.loads(raw)
            require(record.get("schema") == "TI3D-FINAL-FIT-1"
                    and exact(record.get("final_fit_calls"), 1)
                    and type(record.get("training_sample_ids")) is list
                    and len(set(record["training_sample_ids"])) == 50
                    and exact(record["training_sample_ids"], self._training_ids)
                    and exact(record.get("parameters"), RF_PARAMETERS)
                    and exact(record.get("feature_dimension"), 20)
                    and record.get("model_family") == "MULTIMODAL_LBP_RF"
                    and digest_string(record.get("logical_model_sha256")), "FIT_FREEZE_CONTRACT_FAILED")
            self._fit_sha256 = fit_sha256
            self._final_authorized = True
        except (OSError, ValueError, TypeError) as error:
            self._failed = True
            raise ExecutionContractError(getattr(error, "code", "FIT_FREEZE_IO_FAILED")) from None
        finally:
            for value in (fd, parent):
                if value is not None:
                    os.close(value)

    def _start_final_once(self):
        if self._final_started:
            return
        parent, fd = None, None
        try:
            parent, name = _parent_fd(self._root_fd, FINAL_ACCESS_PATH)
            fd = os.open(name, os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW | os.O_CLOEXEC,
                         0o600, dir_fd=parent)
            payload = memoryview(canonical_bytes({
                "schema": "TI3D-FINAL-ACCESS-1", "state": "FINAL_ACCESS_STARTED_IRREVERSIBLY",
                "timestamp_utc": datetime.now(timezone.utc).isoformat(),
                "fit_freeze_sha256": self._fit_sha256, "head_sha": self._head, "final_opens_before_access": 0,
                "consumption_rule": "FIRST_SUCCESSFUL_FINAL_OPEN_CONSERVATIVE_EVEN_ZERO_BYTES",
                "retry_authorized": False}) + b"\n")
            while payload:
                count = os.write(fd, payload)
                require(count > 0, "FINAL_ACCESS_RECEIPT_WRITE_FAILED")
                payload = payload[count:]
            os.fsync(fd)
            os.fsync(parent)
    ...
    def read_native(self, record):
        require(type(record) is dict, "SOURCE_METADATA_REQUIRED")
        if record.get("split") == "FINAL_TEST":
            require(self._final_authorized, "FINAL_BEFORE_FIT_FREEZE_DENIED")
        else:
            require(not self._final_started, "TRAINING_AFTER_FINAL_DENIED")
    ...
    def report(self):
        ...
        result = {"schema": "TI3D-NATIVE-IO-AUDIT-1", "receipt_invocations": 1,
                  ...
                  "final_access_started": self._final_started, "final_consumed": self._final_consumed,
                  "fit_freeze_sha256": self._fit_sha256,
                  "final_consumption_rule": "FIRST_SUCCESSFUL_FINAL_OPEN_CONSERVATIVE_EVEN_ZERO_BYTES"}
