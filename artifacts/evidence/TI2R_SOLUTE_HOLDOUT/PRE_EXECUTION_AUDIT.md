...
PREPARATION_AUDIT_AT_FREEZE=PASS (superseded by the effective preflight below)

## Resultado efetivo do preflight — prevalece sobre a preparação acima

A única invocação do runner retornou código 3 com `CONFIGURATION_DIVERGENCE`.
A igualdade exata distingue tipos: o manifesto preparado pelo agente converteu
seis floats V1 em inteiros. A configuração científica original não mudou.
O erro é operacional (categoria A); a preparação acima não comprovava a
igualdade tipada usada pelo runner e seu PASS foi invalidado.

Não houve receipt, tentativa armada, chamada científica, abertura de holdout
ou leitura experimental. CLI=1, ciência=0, opens=0, bytes=0, retries=0.
Não se corrigiu o manifesto nem se repetiu a invocação.
Holdout permanece SEALED e G2_SOLUTE=BLOCKED_PENDING_LOCKED_HOLDOUT.
É necessária nova decisão do autor para qualquer retomada.

O texto exato anterior ao runner está preservado em
`PRE_EXECUTION_AUDIT_AT_FREEZE.md`; seu hash é o referenciado pelo manifesto
e pelo frozen-hashes original. Esta atualização documental é a única mudança
nos 30 textos originalmente listados após a invocação. Kernels, configuração,
manifesto e dois scripts operacionais permanecem sem alterações.
Os hashes atuais estão em `post-run-hashes.sha256`.
